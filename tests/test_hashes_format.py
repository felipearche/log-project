...
def test_hashes_format_and_crypto():
    ...
    lines = [l for l in HASHES.read_text(encoding="utf-8").splitlines() if l.strip()]
    # exactly four lines
    assert len(lines) == 4, f"Expected 4 lines, got {len(lines)}"
    # two spaces + UPPERCASE HEX; exact four expected paths (any order)
    pat = re.compile(r'^(\d{4}-\d{2}-\d{2})\s{2}(\S+)\s{2}(\d+)\s{2}sha256=([0-9A-F]{64})\s*$')
    expected = {
    ...
    for i, line in enumerate(lines, start=1):
        ...
        assert int(size_s) == actual_size, f"Size mismatch for {path}: {size_s} vs {actual_size}"
    ...
    assert seen == expected, f"Paths mismatch.\nExpected: {sorted(expected)}\nSeen: {sorted(seen)}"
